using System.Text.Json.Serialization;
using Corti.Core;

namespace Corti;

[JsonConverter(typeof(StringEnumSerializer<StreamFlushMessageType>))]
[Serializable]
public readonly record struct StreamFlushMessageType : IStringEnum
{
    public static readonly StreamFlushMessageType Flush = new(Values.Flush);

    public StreamFlushMessageType(string value)
    {
        Value = value;
    }

    /// <summary>
    /// The string value of the enum.
    /// </summary>
    public string Value { get; }

    /// <summary>
    /// Create a string enum with the given value.
    /// </summary>
    public static StreamFlushMessageType FromCustom(string value)
    {
        return new StreamFlushMessageType(value);
    }

    public bool Equals(string? other)
    {
        return Value.Equals(other);
    }

    /// <summary>
    /// Returns the string value of the enum.
    /// </summary>
    public override string ToString()
    {
        return Value;
    }

    public static bool operator ==(StreamFlushMessageType value1, string value2) =>
        value1.Value.Equals(value2);

    public static bool operator !=(StreamFlushMessageType value1, string value2) =>
        !value1.Value.Equals(value2);

    public static explicit operator string(StreamFlushMessageType value) => value.Value;

    public static explicit operator StreamFlushMessageType(string value) => new(value);

    /// <summary>
    /// Constant strings for enum values
    /// </summary>
    [Serializable]
    public static class Values
    {
        public const string Flush = "flush";
    }
}
