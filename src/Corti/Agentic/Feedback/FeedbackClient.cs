...
public partial class FeedbackClient : IFeedbackClient
{
                ...
                var response = await _client
                    .SendRequestAsync(
                        new JsonRequest
                        {
                            ...
                            Path = string.Format(
                                "v2/agentic/contexts/{0}/tasks/{1}/feedback/{2}",
                                ValueConvert.ToPathParameterString(contextId),
                                ValueConvert.ToPathParameterString(taskId),
                                ValueConvert.ToPathParameterString(feedbackId)
                            ),
                            Headers = _headers,
                            Options = options,
                        },
    ...
    )
    {
        return new WithRawResponseTask(
            DeleteAsyncCore(contextId, taskId, feedbackId, options, cancellationToken)
        );
    }
}
